...
for(p in 1:5){
	...
	hist<- hist(MIF, plot =FALSE, breaks= c(0,0.01,0.025,0.05,0.1,0.25,0.3,0.35,0.4,0.45,0.5))
		
	if(p==1){ plot(hist$density, type='l',cex=0.7, col= color[p],log='xy', ylab="Frequency", xlab= "Frequency class",xaxt="n" , main= "Site Frequency Spectrum for each superpopulation\nof the 1000G data for the selected CYPgenes" ) }
